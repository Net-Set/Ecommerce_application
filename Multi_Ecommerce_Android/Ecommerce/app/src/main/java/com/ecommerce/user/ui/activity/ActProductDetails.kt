...
import com.ecommerce.user.adapter.RelatedProductsAdapter
...
class ActProductDetails : BaseActivity() {
    ...
    private var relatedProducts = ArrayList<RelatedProductsItem>()
            ...
            override fun onResponse(
                ...
                if (response.code() == 200) {
                    ...
                    if (restResponce.status == 1) {
                        ...
                        if (restResponce.relatedProducts?.size ?: 0 > 0) {
                            productDetailsBinding.rvstorelist.visibility = View.VISIBLE

                            // restResponce.relatedProducts?.let { loadRelatedProducts(it) }
                            relatedProducts = restResponce.relatedProducts!!
                            val adapter =
                                RelatedProductsAdapter(this@ActProductDetails, relatedProducts)
                            productDetailsBinding.rvstorelist.layoutManager =
                                GridLayoutManager(
                                    this@ActProductDetails,
                                    1,
                            ...
                            productDetailsBinding.rvstorelist.adapter = adapter
                        } else {
                            productDetailsBinding.rvstorelist.visibility = View.GONE
                        }
                        restResponce.vendors?.let { loadVendorsData(it) }

                        productDetailsBinding.clReturnpolicy.setOnClickListener {
                            val intent =
                                Intent(this@ActProductDetails, ActReturnPolicy::class.java)
                            intent.putExtra(
                                "return_policies",
    ...
    private fun loadVendorsData(vendors: Vendors) {
        ...
        if (vendors.rattings == null) {
            ...
            if (vendors.rattings.size > 0) {
                productDetailsBinding.tvvendorsrate.text =
                    vendors.rattings[0].avgRatting?.toString() ?: ""
            } else {
                productDetailsBinding.tvvendorsrate.text = "0.0"
            }
        }
        productDetailsBinding.tvvisitstore.setOnClickListener {
            ...
            if ((vendors.rattings?.size ?: 0) > 0) {
                intent.putExtra("vendors_rate", vendors.rattings?.get(0)?.avgRatting?.toString())
            } else {
                ...
                override fun onBindData(
                    ...
                    binding.tvProductPrice.text =
                        Common.getPrice(
                            ...
                            relatedProducts[position].productPrice!!.toString()

                        )
                    if (relatedProducts[position].discountedPrice == "0" || relatedProducts[position].discountedPrice == null) {
                        binding.tvProductDisprice.visibility = View.GONE
                    } else {
                        binding.tvProductDisprice.text =
                            Common.getPrice(
                                currencyPosition, currency,
                                relatedProducts[position].discountedPrice!!.toString()
                            )
                    }
                    Glide.with(this@ActProductDetails)
    ...
    private fun loadProductDetails(productDetailsList: ProductDetailsData) {
        ...
        productDetailsBinding.tvProductPrice.text =
            Common.getPrice(
                currencyPosition, currency,
                productDetailsList.productPrice!!.toString()
            )


        if (productDetailsList.discountedPrice == "0" || productDetailsList.discountedPrice == null) {
            productDetailsBinding.tvProductDisprice.visibility = View.GONE

        } else {
            productDetailsBinding.tvProductDisprice.text =
                Common.getPrice(
                    currencyPosition, currency,
                    productDetailsList.discountedPrice!!.toString()
                )
        }
        ...
        if (productDetailsList.freeShipping == 1) {
            ...
            productDetailsBinding.tvshoppingcharge.text = currency.plus(
                Common.getPrice(
                    currencyPosition, currency,
                    productDetailsList.shippingCost!!.toString()
                )
            )
        }
        ...
        if (productDetailsList.freeShipping == 1) {
            ...
            productDetailsBinding.tvshoppingchargetitle.text = getString(R.string.freeshipping)
        } else {
        ...
        productDetailsBinding.tvshoppingday.text =
            productDetailsList.estShippingDays + " " + getString(R.string.day)
...
//                        onResume()
    ...
    private fun setupPriceData(data: VariationsItem) {
        ...
        productDetailsBinding.tvProductPrice.text =
            Common.getPrice(
                currencyPosition, currency, data.price.toString()
            )
        productDetailsBinding.tvProductDisprice.text =
            Common.getPrice(
                currencyPosition, currency,
                data.discountedVariationPrice!!.toString()
            )
