...
class ActTrackOrder : BaseActivity() {
    ...
    private fun loadTrackOrderDetails(trackOrderDetailsList: TrackOrderInfo) {
        if (!trackOrderDetailsList.equals(0)) {
            ...
            Log.d("Size",trackOrderDetailsList.variation.toString())

                trackOrderBinding.tvOrderProductSize.text =/*intent.getStringExtra("att").plus(":")+ */intent.getStringExtra("Size")


            val qty = trackOrderDetailsList.qty?.toDouble()
            val price = trackOrderDetailsList.price?.toDouble()
            val totalpriceqty = price!! * qty!!
            Log.d("totalpriceqty",totalpriceqty.toString())
            Log.d("totalpriceqty",qty.toString())
            Log.d("totalpriceqty",price.toString())
